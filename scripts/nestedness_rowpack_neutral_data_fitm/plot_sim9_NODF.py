# plot a histogram of the SES NODF for the simulated data, and compare it to the SES observed
...
dir_simulated = '../../results/nestedness_rowpack_neutral_data_fitm/'

# where the observed data is
dir_observed = '../../results/nestedness_data/'
...
df = pd.read_csv(dir_observed + 'sim9_NODF.csv')
...
# read in all the results
df = pd.read_csv(dir_simulated + 'sim9_NODF.csv')
...
plt.legend(loc='best')
plt.tight_layout()
plt.savefig(dir_simulated + 'sim9_NODF_' + subset + '.pdf')
plt.close()
