...
sample_IDs <- 4:30       # which sample_IDs to do

# which parameter set / suffix to run
suffix <- '_2'

# where samples are stored and the parameter values used to generate them
dir_samples <- '../../results/neutral_vary_JK/'

# where to put the results
dir_results <- '../../results/nestedness_rowpack_neutral_vary_JK/'

# how to do randomisations
burn_in <- 3000 # verified this is adequate with some examples
...
# the island areas are ordered from smallest to largest, so I need to reverse that
# ---
...
# all archipelagos have the same number of islands, so just take the first row
H <- df_params[1,'H']
idx_order <- 16:1 # reverse order is largest to smallest


# get the samples
# ---

fname_samples <- paste( c(dir_samples, 'samples_archipelago', suffix, '.csv'), collapse = '' )
...
for (sample_ID in sample_IDs) {
    ...
    if (plot_histogram) {
        ...
        fname_hist <- paste(
                            c(dir_results, 'sim9_NODF_archipelago', suffix, '_sample_ID_', sample_ID, '.pdf'), 
                            collapse = '')
        pdf(fname_hist)
...
# write statistical results for this K to a csv
# ---
