...
for (subset_name in subset_names){
    ...
    for (sample_ID in sample_IDs) {
        ...
        df <- pack_rows(pamatrix, T)     # sort rows in order from most common to least common species

        # plot observed matrix
        fname_plot <- paste(c(dir_results, 'matrix_archipelago', suffix, '_sample_ID_', sample_ID, '.pdf'), collapse = '')
