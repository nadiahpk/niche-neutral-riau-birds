...
island2niches = {
        'lingga'        : [0, 1, 2, 3, 4, 5, 6, 7, 8],
        'sebangka'      : [4, 5, 6, 7, 8, 9],
        'temiang'       : [3, 4, 5, 6, 7, 8, 9],
        'durian'        : [1, 2, 3, 4, 6, 7, 8, 9],
        'ngenang'       : [1, 2, 3, 4, 5, 6, 7, 8, 9],
...
max_K = 10 # how many niches total
...
# save datafrmae
fname_csv = dir_results + 'archipelago_params' + suffix + '.csv' # NOTE suffix change here
df_out.to_csv(fname_csv, mode='w', header=True, index=False)

if True: # print to screen what was created
    ...
    vv = sorted(list(zip(island_names, areas, S_truV, S_new, K_new, m_new)), key = lambda x: x[1], reverse=True)
    print(vv)
