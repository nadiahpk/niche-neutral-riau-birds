...
sample_IDs <- 1:30       # which sample_IDs to do
...
dir_results <- '../../results/nestedness_rowpack_neutral_vary_K/'
...
fname_samples <- paste( c(dir_samples, 'samples', suffix, '_archipelago_', archipelago_ID, '.csv'), collapse = '' )
...
for (sample_ID in sample_IDs) {
    ...
    if (plot_histogram) {
        ...
        fname_hist <- paste(
                            c(dir_results, 'sim9_NODF', suffix, '_archipelago_', archipelago_ID, '_sample_ID_', sample_ID, '.pdf'), 
                            collapse = '')
        pdf(fname_hist)
...
fname_out <- paste(c(dir_results, 'sim9_NODF', suffix, '.csv'), collapse = '')
