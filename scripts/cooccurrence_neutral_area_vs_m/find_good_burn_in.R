...
dir_samples <- '../../results/neutral_area_vs_m/'
...
df_samples <- subset(df_samples, archipelago_ID == archipelago_ID)
row <- df_samples[1,]
...
# make the plots
fname_out <- paste( c(dir_results, 'burn_in', suffix, '_archipelago_ID_', archipelago_ID, '.pdf'), collapse='' )
pdf(fname_out)
plot(result, type = 'burn_in')
dev.off()

# save the final matrix to a file
fname_out <- paste( c(dir_results, 'burnt_in', suffix, '_archipelago_ID_', archipelago_ID, '.csv'), collapse='' )
...
cat('Effective sample size of ', archipelago_ID, ': ', effectiveSize(sample), '\n')
# ~ 9
