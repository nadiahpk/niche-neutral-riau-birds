...
# for each K, compare the observe NODF to randomised
# ---

for (K in KV) {

    print(paste( c('doing K = ', K), collapse='' ))
    ...
    for (sample_ID in sample_IDs) {
        ...
        if (plot_histogram) {
            ...
            fname_hist <- paste(
                                c(dir_results, 'sim9_NODF', suffix, '_K_', K, '_sample_ID_', sample_ID, '.pdf'), 
                                collapse = '')
            pdf(fname_hist)
    ...
    fname_out <- paste(c(dir_results, 'sim9_NODF', suffix, '_K_', K, '.csv'), collapse = '')
