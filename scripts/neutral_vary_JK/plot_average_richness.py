...
SV = [ np.mean(df['no_spp_isle_' + str(h)].values) for h in range(H) ]
...
plt.tight_layout()
plt.savefig(dir_results + 'average_richness_archipelago' + suffix + '.pdf')
plt.close()
...
df_out.to_csv( dir_results + 'average_richness_archipelago' + suffix + '.csv', index=False)
