...
for (arch_ID in arch_IDs) {
    ...
    for (sample_ID in sample_IDs) {
        ...
        if (plot_histogram) {
            ...
            fname_hist <- paste(
                                c(dir_results, 'histogram_c_score/sim9_c_score', suffix, '_archipelago_', arch_ID, '_sample_ID_', sample_ID, '.pdf'), 
                                collapse = '')
            pdf(fname_hist)
    ...
    fname_out <- paste(c(dir_results, 'sim9_c_score', suffix, '.csv'), collapse = '')
