# plot a histogram of the SES C-scores for the simulated data
...
dir_simulated = '../../results/cooccurrence_neutral_vary_K/'
...
df = pd.read_csv(dir_simulated + 'sim9_c_score' + suffix + '.csv')

# subset out the results we're interested in
df = df[df['archipelago_ID'] == archipelago_ID]
...
plt.legend(loc='best')
plt.tight_layout()
plt.savefig(dir_simulated + 'sim9_c_score' + suffix + '_archipelago_' + str(archipelago_ID) + '.pdf')
plt.close()
